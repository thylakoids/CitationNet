...
def getdata(seednode='27635430'):
	...
	while not q_all.empty() and len(L_get)<100000:############<1000
		...
		while(flag):
			try:
				flag=0
				citationList = get_citation_id(mynode)
			except Exception, e:
				flag =1
				count_try+=1
	...
	arrows=['to']*len(fromlist)
	output_edges=pd.DataFrame({'from':fromlist,'to':tolist,'arrows':arrows})
	#output_edges.to_csv('edges1.csv',index=False)
	fromlist.extend(tolist)
	fromlist=list(set(fromlist))
	titles=[get_summary(x)['Title'] for x in fromlist]
	label=[str(x) for x in fromlist]
	output_nodes=pd.DataFrame({'id':fromlist,'label':label,'title':titles})
